defmodule Ecto.Query.Planner do
  ...
  defp plan_from(
         %{from: %{source: {:fragment, _, _}}, preloads: preloads, assocs: assocs} = query,
         _adapter
       )
    ...
    {queries, counter} =
      Enum.reduce(with_expr.queries, {[], counter}, fn
        {name, %Ecto.Query{} = inner_query}, {queries, counter} ->
          ...
          fields =
            cte_fields(Keyword.keys(types), Enum.reverse(fields), inner_query.select.aliases, [])
  ...
  defp collect_fields(
         {:selected_as, _, [select_expr, _name]} = expr,
         fields,
         from,
         query,
         take,
  ...
  defp cte_fields(
         [key | rest_keys],
         [{:selected_as, _, [select_expr, key]} | rest_fields],
         aliases,
         acc
       ) do
